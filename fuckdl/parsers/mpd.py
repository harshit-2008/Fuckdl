        ...
        for adaptation_set in period.findall("AdaptationSet"):
            ...
            for rep in adaptation_set.findall("Representation"):
                ...
                def get_track_size(track_repr):
                    ...
                    if segment_list:
                        file_size = sorted(segment_list[0].findall('SegmentURL'), key=lambda segment_url: int(segment_url.get('mediaRange').split('-')[1]), reverse=True)
                        if file_size:
                            num_bytes = int(file_size[0].get('mediaRange').split('-')[1])    # bytes
