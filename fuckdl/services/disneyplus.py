...
from fuckdl.objects import MenuTrack, Title, Tracks, AudioTrack, VideoTrack
from fuckdl.services.BaseService import BaseService
from fuckdl.utils.BamSDK import BamSdk
...
class DisneyPlus(BaseService):
    ...
    def license(self, challenge, **_):
        if self.cdm.device.type == LocalDevice.Types.PLAYREADY:
            ...
            r = requests.post(
                ...
                headers={'authorization':self.account_tokens.get("accessToken") or self.account_tokens.get("access_token")},
                data=challenge
            )
            ...
            return self.bamsdk.drm.widevineLicense(
                ...
                access_token=self.account_tokens.get("accessToken") or self.account_tokens.get("access_token")
            )
    ...
    def get_manifest_tracks(self, url):
        ...
        for audio in tracks.audios:
            ...
            bitrate = re.search(r"(?<=r/composite_)\d+|\d+(?=_complete.m3u8)|\d+(?=_complete)|\d+(?=_hdri_complete)", as_list(audio.url)[0])
            if not bitrate:
