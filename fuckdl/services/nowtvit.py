...
class NowTVIT(BaseService):
    """
    \b
    Service code for Now TV's streaming service (https://nowtv.it)
    ...
    ALIASES = ["NOWTV", "nowtvit", "NowTV", "NowTv", "Nowtv", "nowtv"]
    #GEOFENCE = ["it"]

    TITLE_RE = [
    r"https?://(?:www\.)?nowtv\.[a-z]+/watch/asset(?:/movies|/tv)?(?P<id>/[._a-z0-9-]+/[a-zA-Z0-9_]+)",
    r"https?://(?:www\.)?nowtv\.[a-z]+/watch/asset/(?P<id>/[._a-z0-9-]+/[a-zA-Z0-9_]+)"
    ]


    @staticmethod
    @click.command(name="NowTVIT", short_help="https://nowtv.com", help=__doc__)
    @click.argument("title", type=str)
    @click.option("-m", "--movie", is_flag=True, default=False, help="Title is a movie.")
    @click.pass_context
    def cli(ctx, **kwargs):
        return NowTVIT(ctx, **kwargs)
    ...
    def get_titles(self):
        ...
        res = self.session.get(
            url="https://eu.api.atom.nowtv.it/adapter-calypso/v3/query/node?slug="+self.title,
